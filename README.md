...
- **Режим**:
  ...
  - *Легитный* — повороты проходят через `RotationHumanizer` (тот же слой, что и в легитном
    AutoMine): плавный доворот с промахом и переменной скоростью, удар — только когда прицел
    действительно наведён (до 6°), плюс человеческие паузы при смене цели и между ударами;
