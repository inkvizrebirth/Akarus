...
| **AutoBuff** | — | Авто-зелья в бою: пьёт **до конца** (удержание использования с таймаутом), режимы fast/legit; баффы Скорость/Сила/Огнестойкость/Регенерация/Ночное зрение; при низком HP — мгновенное лечение или золотое яблоко |
...
Клиентская точка входа (`"client"` в fabric.mod.json), 14 миксинов без refmap'а
