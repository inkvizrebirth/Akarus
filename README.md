...
1. **GitHub Actions (рекомендуется, если локально нет JDK 25 или нет доступа к Gradle):**
   ...
   например [Akarus v0.6.0](https://github.com/inkvizrebirth/Akarus/releases/tag/v0.6.0),
   файл `akarus-0.6.0.jar`.
...
| **KillAura** | Бой | `X` | Автоматическая атака: режимы **Быстрый**/**Легитный**, приоритет цели, дальность, Авто-Блок щитом, Смарт Крит, сброс спринта, обязательный RayTrace |
| **Спринт** | Движение | `M` | Всегда держит спринт: бежит, как только игрок идёт вперёд |
| **NoFOV** | Рендер | — | Статичный FOV: угол обзора не меняется от спринта и эффектов (значение настраивается) |
| **NoBlind** | Рендер | — | Убирает визуальные помехи: тошноту, тьму и слепоту (настраивается по эффектам) |
| **NoFallDamage** | Движение | — | Ватердроп: ставит ведро воды под себя при опасном падении, после посадки убирает воду |
...
- **Авто-Блок (щит)** — пока враг смотрит на нас и стоит в зоне досягаемости удара (до 3.2 блока),
  держим щит (вторая рука, при наличии — основная). Как только враг замахнулся (ударил по щиту) —
  щит сбрасывается и следует контрудар, после чего щит поднимается снова. Щит не поднимается,
  пока он у нас на кулдауне (после топора);
- **Смарт Крит** — вместо удара с земли: прыгаем, ждём окна крита (падение) и бьём критом,
  после чего щит возвращается сразу. Если крит невозможен (вода, лестница, полёт) — обычный удар;
- **Сброс спринта** — Выкл / **Быстрый** / Легитный. Быстрый: на тик снимается спринт — полный
  нокбэк каждым ударом, максимум эффективности. Легитный: человеческий w-tap — на 1–2 тика
  отпускаются W и спринт, как будто игрок сам передвинул пальцы;
- **Коррекция движений** — как игрок двигается, пока аура целится:
  - *Свободный* — ввод пересчитывается так, будто камеры ауры нет: **W ведёт туда, куда смотрел
    сам игрок**, а не куда наведён прицел (углы доворотов на ауру не влияют);
  - *Фокусированный* — аура сама ведёт игрока: W до цели, у центра (≤2.2 блока) — стрейф-орбита
    со сменой направления каждые 8–14 тиков и «ультра-быстрое кружение» камерой (~38–52°/тик);
    удары проходят по RayTrace на каждом обороте. В этом режиме повороты всегда быстрые;
  - *Легитный* — аура клавиши не трогает: игрок идёт сам и только пока **сам держит W** (к цели —
    камера уже наведена); после удара на тик W отпускается (w-tap).

**Обязательный RayTrace** (всегда включён): удар уходит только если луч из глаз по текущему
взгляду реально пересекает хитбокс цели (с допуском 0.15 блока) — ровно эту проверку делает
сервер и античиты. Пока прицел доворачивается, aura не бьёт «в молоко».

**Движение за камерой** (легитный режим): игрок идёт туда, куда фактически смотрит камера
киллауры — W удерживается, пока цель дальше 1.4 блока; вблизи отпускается, чтобы не толкать цель,
а после удара на пару тиков W отпускается (легитный сброс спринта).
...
приостанавливается (но щит, который она держит, не опускается). Модуль «Спринт»-aware: на время
сброса спринта киллаура просит его не мешать.

### Спринт

Всегда держит спринт (`M`): клавиша спринта удерживается, а сам флаг подтягивается вручную,
как только игрок действительно идёт вперёд (`zza > 0`) — ваниль иногда «теряет» спринт после
удара или короткой заминки, с модулем бег возобновляется сам.

- во время еды/натягивания лука спринт не форсируется (это выглядело бы подозрительно);
- FreeCam не конфликтует: пока он активен, игрок и так стоит;
- Baritone не мешает: он управляет движением своими флагами поверх клавиши;
- киллаура на 1–2 тика снимает спринт для сброса (w-tap) — модуль в это время сознательно
  молчит и возвращает спринт сразу после.

### NoFOV

Фиксирует угол обзора: `Camera#getFov` всегда возвращает выбранное значение (30–110, по умолчанию
90). Спринт, скорость, лук, еда, вода и смерть больше не «дышат» картинкой. FOV рук (отдельная
проекция) не трогается.

### NoBlind

Убирает визуальные помехи, каждый эффект — отдельной настройкой:

- **Тошнота** — постэффект nausea (крутит и размывает экран) подменяется на «ничего»;
- **Тьма и слепота** — эффекты снимаются с игрока на клиенте каждый тик: сервер продолжает их
  считать, но экран остаётся чистым;
- **Огонь на экране** — пламя при горении больше не застилает вид.

Чисто клиентская косметика: ни одного пакета не меняется.

### NoFallDamage (Ватердроп)

Как только падение становится опасным (`fallDistance ≥ 3`), модуль находит ведро воды (вторая рука →
хотбар → основной инвентарь, откуда ведро shift-кликом переезжает в хотбар), берёт его в руку и
ставит воду под ноги. Прежний слот запоминается и восстанавливается.

**Автоматически убирать воду** — включено по умолчанию: вода собирается обратно в ведро **только**
после подтверждённой посадки (игрок в воде, `fallDistance` сброшен) плюс ещё 6 тиков запаса.
Сразу после установки вода не убирается; если игрок в неё не попал — по таймауту вода убирается
лишь после приземления (чистка за собой), а пока полёт продолжается — модуль просто ждёт.

Вода/лестница/элитры/пассажиры не считаются опасным падением. Ведро после сбора возвращается
в инвентарь полным — цикл можно повторять.
