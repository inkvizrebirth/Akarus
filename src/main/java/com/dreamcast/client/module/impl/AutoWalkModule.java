...
public class AutoWalkModule extends Module {
	...
	protected void onEnable() {
		...
		if (!BaritoneBridge.isAvailable()) {
			...
			DreamcastClient.LOGGER.warn("Baritone не найден, AutoWalk отключается");
	...
	public void tick() {
		...
		if (distance <= 1.5) {
			notify("§a[Dreamcast] Пришёл: §f" + format(target));
			setEnabled(false);
			return;
		}
		...
		if (autoDisable.isEnabled() && walkingTicks > 20 && !BaritoneBridge.isPathing() && distance > 2.5) {
			notify("§e[Dreamcast] Baritone прекратил путь — выбираю точку заново");
			cancelRoute();
		}
	}
	...
	private void onRightClick() {
		...
		if (position == null) {
			notify("§c[Dreamcast] Ни во что не смотрю — наведи камеру на место, куда нужно дойти");
			return;
		}
