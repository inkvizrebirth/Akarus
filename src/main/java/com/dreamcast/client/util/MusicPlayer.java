...
public final class MusicPlayer {
	...
	private void startLoadLocked(File file, boolean autoplay) {
		...
		Thread loader = new Thread(() -> loadOnWorker(target, myGeneration, start), "dreamcast-media-loader");
	...
	private void loadOnWorker(File file, int myGeneration, boolean start) {
		...
		try (AudioInputStream source = AudioSystem.getAudioInputStream(file)) {
			...
			DreamcastClient.LOGGER.info("Медиаплеер: загружен {}", file.getName());
			...
			DreamcastClient.LOGGER.warn("Медиаплеер: не удалось открыть {}", file, exception);
		}
	}
