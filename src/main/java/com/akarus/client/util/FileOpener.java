...
public final class FileOpener {
	...
	public static void openFolder(Path folder) {
		try {
			...
			AkarusClient.LOGGER.warn("Не удалось создать папку {}", folder, exception);
		}
	...
	private static void open(File target) {
		...
		try {
			...
			AkarusClient.LOGGER.warn("Не удалось открыть {} через системный проводник", target, exception);
		}
	}
}
